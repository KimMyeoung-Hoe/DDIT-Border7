		...
		<main class="main-content-area">
			<div class="consultant-wrap">
				<div class="flex_div controls">
					<input type="hidden" id="roomIdInput" placeholder="회의실" value="${nobang }" />
					<button type="button" id="enterRoomBtn" style="display: none;">입장하기</button>
					<button type="button" id="startSteamBtn">화면 켜기</button>
					<button type="button" id="endSteamBtn">화면 끄기</button>
				</div>
				
				<div class="video-ele-wrap">
					<div id="localStream" class="video-container"> <video autoplay playsinline controls muted></video> </div>
					<div id="remoteStreamDiv" class="video-container"></div>
				</div>
...
const startCam = async () => {
    if(navigator.mediaDevices !== undefined){
        try {
            ...
            alertify.alert("카메라 또는 마이크 접근에 실패했습니다. 권한을 확인해주세요.");
        }
    }
}
...
const connectRoomWebSocket = async () => {
    if (!roomId) {
        alertify.alert("회의실 번호를 먼저 입력해주세요.");
        return;
    }
    ...
    roomStompClient.connect({}, function (frame) {
       ...
       roomStompClient.subscribe("/topic/chat/leave/" + roomId, message => {
		    ...
		    if (pcListMap.has(leaverKey)) {
		        ...
		        displayChatMessage("[알림] " +leaverKey, leaveMessage);
		    }
		    getLeaveEvent();
       });
       ...
       roomStompClient.subscribe("/topic/stream/end/"+roomId, message => {
        ...
        alertify.alert("회의실 연결에 실패했습니다. 다시 시도해주세요.");
    });
}
...
const displayChatMessageIcon = (sender, message) => {
	let ele = "";
	if(myKey === sender){
		ele +='<div class="message-row mymessage" data-sender='+sender+'>';
		...
		ele +='<div class="message-row send" data-sender='+sender+'>';
	    ele +='    <div class="profile-image-wrapper">';
	    ele +='        <i class="fas fa-user"></i>';
	    ele +='    </div>';
	    ele +='    <div class="message-flex">';
...
const sendChatMessage = () => {
	...
	if(!roomStompClient || !roomStompClient.connected){ // roomStompClient 확인
		alertify.alert("회의실 웹소켓 연결 중이 아닙니다. 회의실에 입장해주세요.");
		return false;
	}
	if(!roomId){
		alertify.alert("회의실 번호를 입력하고 입장해주세요.");
...
document.addEventListener("DOMContentLoaded", async function(){ // async 추가
    ...
    setTimeout(async () => {
        if (roomStompClient && roomStompClient.connected) {
            ...
            alertify.alert("웹소켓 연결에 실패했습니다. 페이지를 새로고침 해주세요.");
