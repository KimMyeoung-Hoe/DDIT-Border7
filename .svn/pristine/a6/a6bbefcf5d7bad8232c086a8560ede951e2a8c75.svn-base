		   ...
		   <div class="breadcrumb-warp">
		      <div class="col-sm-12">
		         <ol class="breadcrumb">
		            ...
		            <li class="breadcrumb-item"><a href="/servant/customsHistory.do">수출입 통관 이력</a></li>
		         </ol>
		      </div>
		      <!-- 브레드크럼 엘리먼트 끝 -->
		   </div>
		   <div class="content-title">수출입 통관 이력</div>
		   <p class="desc">필증발행 및 반려를 통해 종료된 통관신청의 이력을 조회하는 페이지 입니다.</p>
		</div>
		<div class="section">
			<!-- 검색 엘리먼트 -->
				...
				<table class="data-table">
			        ...
			        <tbody>
						<c:choose>
							...
							<c:otherwise>
							    <c:forEach var="customs" items="${pagingVO.dataList}">
							        <tr id="tr_index_${status.index}">
									    ...
									    <td id="td_middleCategory_${status.index}"
									    	<c:choose>
										...
										<td id="td_date_${status.index}">
											<c:set var="declRealDate" value="${customs['DECLREALDATE']}" />
											<fmt:formatDate value="${declRealDate}" pattern="yyyy-MM-dd" />
										</td>
							        </tr>
							    </c:forEach>
							</c:otherwise>
...
$(document).ready(function () {
	let baseUrl = "/servant/customsHistory.do"; 
