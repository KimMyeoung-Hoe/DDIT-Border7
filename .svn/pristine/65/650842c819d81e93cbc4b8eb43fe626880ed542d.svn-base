...
public class MainPageController {
	...
	@GetMapping("/")
	public String main(@RequestParam(required = false)@DateTimeFormat(pattern = "yyyy-MM-dd") Date startDate,
        ...
        if("ROLE_LOGISTICS".equals(loginAuth)) {
        	...
        	return "roleMain/ROLE_LOGISTICS";
        	
        }
        
        if("ROLE_CONSIGNOR".equals(loginAuth)) {
        		...
        		//다진 추가
        			int userNo = user.getUserNo();
                	List<ContractVO> consignorContractList = mainService.consignorContractList(userNo);
                	model.addAttribute("contractList",consignorContractList);
                
              
        	return "roleMain/ROLE_CONSIGNOR";
        }
